...
fortRole = 897960344725635112


async def sFort(ctx, link, discordClient, command):
    isHighRank = False
    ...
    notes = str(ctx.message.content).removeprefix(f"rr!fort {link} ")
    if len(notes) == 0:
    ...
    fairzoneEmbed = discord.Embed(
        title="FORT REQUEST",
        url=link,
        description=f"<@{ctx.message.author.id}> is requesting for somebody to raid their fort.\n```{notes}```",
        color=16711680
    )
    ...
    await channel.send(f"<@&{fortRole}> {link}", embed=fairzoneEmbed)
